# -*- coding: utf-8 -*-
"""
Created on Fri Apr 13 20:18:49 2018
...
colon = pd.concat([colon,rectum_other]) #colon cancer data set
rectum = pd.concat([rectum1,rectum2]) # rectum cancer data set
#rectum = rectum.reset_index(drop=True)
del rectum1,rectum2,rectum_other,data #release the memory 

'''comorbidity counter '''
other_category = list(other_cancer_PID.columns) #list the all patient
other_category.remove('Colon') #because the main cancer is colon, so del it
...
select_com = rectum['PATIENT ID NUMBER']
in_cancer_select(select_com)

#for i in other_category: # 8 iteration
#    in_cancer(i)
#    print(i)
...
colon['REGIONAL NODES POS RATIO'] = round(colon['REGIONAL NODES POSITIVE'] / colon['REGIONAL NODES EXAMINED'],2)
...
colon = colon.drop(Race_group3+group1,axis=1)
'''model testing'''
...
print('pre-processed:',len(colon))

#col_counts = []
#for i in remain_columns:
#    col_counts.append(colon[i].value_counts())

colon.to_csv('For_Model.csv' ,index = False ,encoding='utf-8')
